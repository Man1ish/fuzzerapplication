...
trait Logging {
  ...
  //Write a function that reads a file and save the content to the file
  ...
  def emit(loglevel: LogLevel, id: TransactionId, from: AnyRef, message: => String) = {
    if (loggingAdapter.isEnabled(loglevel)) {
      ...
      if (logmsg.nonEmpty) { // log it only if its not empty
        ...
        //          "loglevel" -> level,
        //          "message" -> logMessage,
        //          "transaction_id" -> id.root,
        //          "component" -> name)
        //        val jsonString: String = jsonMap.toString()
        //        val data = "{\"timestamp\": \"" + time + "\"}"
        val data = s"""{"timestamp": "$time", "message": "$logmsg", "component": "$name","loglevel": "$level", "transaction_id": "$id.root"}"""


        TrackLogger.saveLog(data)
...
object LoggingMarkers {
  ...
  //New log
  private val manualPoolLog = "manualPoolLog"
